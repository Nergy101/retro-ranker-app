...
import { Box, Text, ScrollView, VStack, HStack, Pressable, Image, Button } from 'native-base';
...
const exampleComparisons: ExampleComparison[] = [
  { deviceA: 'steam-deck-oled', deviceB: 'switch-2', label: 'Steam Deck OLED vs Switch 2' },
  { deviceA: 'rg-477m', deviceB: 'rg-476h', label: 'RG 477M vs RG 476H' },
  { deviceA: 'retroid-pocket-flip-2', deviceB: 'thor', label: 'Retroid Pocket Flip 2 vs Thor' },
];
...
export default function ComparePage() {
  ...
  const [exampleDevices, setExampleDevices] = useState<{ [key: string]: Device }>({});
  const [loadingExamples, setLoadingExamples] = useState(true);
  const [isColorExplanationExpanded, setIsColorExplanationExpanded] = useState(false);
    ...
    <GestureDetector gesture={swipeGesture}>
      <Box flex={1} bg={colors.background}>
        <ScrollView>
          ...
          {/* Example Comparisons */}
          {!loadingExamples && devices.length === 0 && (
            <Box mb={4}>
              <Text fontSize="md" fontWeight="semibold" color={colors.textSecondary} mb={3}>
                Popular Comparisons
              </Text>
              <VStack space={3}>
                      ...
                      {({ isPressed }) => (
                        ...
                        >
                          <HStack space={3} alignItems="center">
                            <Image
                              source={{ uri: getDeviceImageUrl(deviceA) }}
                              alt={deviceA.name.raw}
                              width={50}
                              height={50}
                              resizeMode="contain"
                              ...
                              borderRadius="md"
                            />
                            <Text fontSize="lg" color={colors.primary} fontWeight="bold">
                              vs
                            </Text>
                            <Image
                              source={{ uri: getDeviceImageUrl(deviceB) }}
                              alt={deviceB.name.raw}
                              width={50}
                              height={50}
                              resizeMode="contain"
                              bg={colors.backgroundElevated}
                            ...
                            />
                            <VStack flex={1} ml={2}>
                              <Text fontSize="sm" color={colors.textPrimary} fontWeight="semibold">
                                {example.label}
                              </Text>
                              <Text fontSize="xs" color={colors.textSecondary}>
                                {deviceA.brand.raw} {deviceA.name.raw} vs {deviceB.brand.raw} {deviceB.name.raw}
                              </Text>
                            </VStack>
                          </HStack>
                        </Box>
          ...
          {devices.length === 2 && ranking && (
            <>
              ...
              <Box bg={colors.backgroundCard} p={4} borderRadius="md" mb={4}>
                <Pressable onPress={() => setIsColorExplanationExpanded(!isColorExplanationExpanded)}>
                ...
                {isColorExplanationExpanded && (
                  <VStack space={2} mt={3}>
                    <HStack space={2} alignItems="center">
                      <Box bg={colors.success} borderRadius="full" width={20} height={20} />
                      <Text fontSize="sm" color={colors.textPrimary}>
                        Green = Better in this category
                      </Text>
                    </HStack>
                    <HStack space={2} alignItems="center">
                      <Box bg={colors.info} borderRadius="full" width={20} height={20} />
                      <Text fontSize="sm" color={colors.textPrimary}>
                        Blue = Equal in this category
                      </Text>
                    </HStack>
                    <HStack space={2} alignItems="center">
                      <Box bg={colors.error} borderRadius="full" width={20} height={20} />
                      <Text fontSize="sm" color={colors.textPrimary}>
              ...
              <VStack space={4}>
                {devices.map((device) => (
                  <DeviceComparisonResult key={device.id} device={device} ranking={ranking} />
                ))}
              </VStack>
            </>
          )}
          ...
          {devices.length === 0 && (
            <Box bg={colors.backgroundCard} p={4} borderRadius="md">
              <Text color={colors.textSecondary} textAlign="center">
                Select two devices above to compare their specifications side-by-side.
              </Text>
            </Box>
          )}
          </VStack>
