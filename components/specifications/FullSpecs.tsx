...
export function FullSpecs({ device }: FullSpecsProps) {
  ...
  const renderSpecRow = (label: string, value: string | null | undefined) => {
    ...
    return (
      <HStack justifyContent="space-between" py={1.5} borderBottomWidth={1} borderBottomColor={colors.border}>
        <Text color={colors.textSecondary} fontSize="sm" flex={1}>
          {label}
        </Text>
        <Text color={colors.textPrimary} fontSize="sm" flex={2} textAlign="right">
          {value}
        </Text>
      </HStack>
    );
  ...
  return (
    <Box bg={colors.backgroundCard} p={4} borderRadius="md">
      <Pressable onPress={() => setIsExpanded(!isExpanded)}>
        <HStack justifyContent="space-between" alignItems="center" mb={isExpanded ? 4 : 0}>
      ...
      {isExpanded && (
        ...
        <Box>
          ...
          <VStack>
            ...
            {device.os.customFirmwares.length > 0 && 
              renderSpecRow('Custom Firmware', device.os.customFirmwares.join(', '))}
              ...
              {device.cpus?.map((cpu, idx) => (
                <React.Fragment key={idx}>
                  ...
                  {cpu.clockSpeed && renderSpecRow(
                    'CPU Clock Speed',
                    `${cpu.clockSpeed.min ? `${cpu.clockSpeed.min}-` : ''}${cpu.clockSpeed.max} ${cpu.clockSpeed.unit}`
                  )}
              ...
              {device.gpus?.map((gpu, idx) => (
                <React.Fragment key={idx}>
                  ...
                  {gpu.clockSpeed && renderSpecRow(
                    'GPU Clock Speed',
                    `${gpu.clockSpeed.min ? `${gpu.clockSpeed.min}-` : ''}${gpu.clockSpeed.max} ${gpu.clockSpeed.unit}`
                  )}
        ...
        <Box>
          ...
          <VStack>
            {renderSpecRow('RAM', device.ram ? `${device.ram.sizes?.join(', ') || ''} ${device.ram.unit || ''} ${device.ram.type || ''}`.trim() : null)}
        ...
        {device.screen && (
          <Box>
            ...
            <VStack>
              {renderSpecRow('Screen Size', device.screen.size ? `${device.screen.size}"` : null)}
              {device.screen.resolution && device.screen.resolution.length > 0 && 
                renderSpecRow('Resolution', device.screen.resolution.map(r => `${r.width}x${r.height}`).join(', '))}
              ...
              {device.screen.ppi && renderSpecRow('PPI', device.screen.ppi.join(', '))}
            </VStack>
          </Box>
        )}
        ...
        {device.battery && (
          <Box>
            ...
            <VStack>
              {renderSpecRow('Battery Capacity', device.battery.capacity ? `${device.battery.capacity} ${device.battery.unit || ''}` : null)}
              {renderSpecRow('Battery Type', device.battery.type)}
              {renderSpecRow('Removable', device.battery.removable ? 'Yes' : 'No')}
        ...
        {device.controls && (
          <Box>
            ...
            <VStack>
              {renderSpecRow('D-Pad', device.controls.dPad?.type || device.controls.dpad)}
              {device.controls.analogs && renderSpecRow(
                'Analog Sticks',
                `${device.controls.analogs.dual ? 'Dual' : device.controls.analogs.single ? 'Single' : 'None'}${device.controls.analogs.isHallSensor ? ' (Hall Sensor)' : ''}`
              )}
              {renderSpecRow('Face Buttons', device.controls.faceButtons || device.controls.numberOfFaceButtons?.toString())}
        ...
        {device.connectivity && (
          <Box>
            ...
            <VStack>
              {renderSpecRow('WiFi', device.connectivity.hasWifi ? 'Yes' : 'No')}
              {renderSpecRow('Bluetooth', device.connectivity.hasBluetooth ? 'Yes' : 'No')}
              {renderSpecRow('USB-C', device.connectivity.hasUsbC ? 'Yes' : 'No')}
              {renderSpecRow('USB', device.connectivity.hasUsb ? 'Yes' : 'No')}
              {renderSpecRow('NFC', device.connectivity.hasNfc ? 'Yes' : 'No')}
            </VStack>
          </Box>
        )}
        ...
        <Box>
          ...
          <VStack>
            ...
            {renderSpecRow('Weight', device.weight ? `${device.weight}g` : null)}
            {renderSpecRow('Shell Material', device.shellMaterial?.raw)}
            {device.colors.length > 0 && renderSpecRow('Colors', device.colors.join(', '))}
          </VStack>
        </Box>
      </VStack>
      )}
