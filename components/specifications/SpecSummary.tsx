...
export function SpecSummary({ device }: SpecSummaryProps) {
  const specs = [
    ...
    },
    {
      label: 'CPU',
      value: device.cpus?.[0] 
        ? `${device.cpus[0].cores || 'Unknown'} cores @ ${device.cpus[0].clockSpeed?.max || 'Unknown'} ${device.cpus[0].clockSpeed?.unit || ''}`
        : 'Unknown',
    },
    {
      label: 'GPU',
    ...
    },
    {
      label: 'RAM',
      value: device.ram 
        ? `${device.ram.sizes?.join(', ') || 'Unknown'} ${device.ram.unit || ''}`
        : 'Unknown',
    },
    {
      label: 'Display',
      value: device.screen
        ? `${device.screen.size || 'Unknown'}" ${device.screen.resolution && device.screen.resolution.length > 0
            ? device.screen.resolution.map((res) => `${res.width}x${res.height}`).join(', ')
    ...
    },
    {
      ...
      value: device.battery
        ? `${device.battery.capacity || 'Unknown'} ${device.battery.unit || ''} (${device.battery.type || 'Unknown'})`
        : 'Unknown',
    },
    {
      label: 'Dimensions',
    ...
    <Box bg={colors.backgroundCard} p={4} borderRadius="md">
      ...
      <VStack space={2}>
        {specs.map((spec, index) => (
          <HStack key={index} justifyContent="space-between" alignItems="flex-start">
            <Text 
